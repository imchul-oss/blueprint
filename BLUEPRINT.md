...
| `npm run mcp` | 컴파일된 MCP 서버 stdio 시작 (도구 22개) |
| `npm run demo` | 컴파일된 데모 1회 실행 |
| `npm run smoke` | 환경 무관 스모크 검증 (24 케이스) |
| `npm run test:mcp:all` | MCP 로직(7) + 실 stdio e2e (도구 표면 포함) |
| `npm run test:purpose` | 목적 단위 e2e — 웹↔MCP 동시 편집 단일진실원천 (9 케이스) |
| `npm run test:features` | 피처 e2e — delta read·첨부 서빙·drift·자가승인 차단·md 왕복 (12 케이스) |
| `npm test` | vitest 단위 테스트 (node_modules 정상 시 — 환경 이슈 시 smoke 로 대체) |
...
├── audit.jsonl          # append-only 감사 (10MB 초과 시 날짜 파일로 자동 로테이션)
└── snapshots/
    └── r00007-abc123def.json  # confirm 시점별 사본 (50개 보관, compact JSON)
